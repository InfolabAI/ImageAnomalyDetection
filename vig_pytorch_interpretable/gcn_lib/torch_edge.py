...
def include_reverse(training, dist, k, is_include: str = 'no'):
    if is_include == "no":
        _, nn_idx_part = torch.topk(-dist, k=k)
        ret = nn_idx_part
        # logger.info("no")
    elif is_include == "only_reverse":
        _, nn_idx_part = torch.topk(dist, k=k)
        ret = nn_idx_part
        # logger.info("no")
    elif is_include == "training_only":
        if training:
            _, nn_idx_part1 = torch.topk(-dist, k=k//2+k % 2)
            _, nn_idx_part2 = torch.topk(dist, k=k//2)
            ret = torch.concat([nn_idx_part1, nn_idx_part2], dim=2)
            # logger.info("training_only yes")
        else:
            _, nn_idx_part = torch.topk(-dist, k=k)
            ret = nn_idx_part
            # logger.info("training_only no")

    elif is_include == "always":
        _, nn_idx_part1 = torch.topk(-dist, k=k//2+k % 2)
        _, nn_idx_part2 = torch.topk(dist, k=k//2)
        ret = torch.concat([nn_idx_part1, nn_idx_part2], dim=2)
        # logger.info("always yes")
    else:
        raise ValueError(f"Invalid is_include: {is_include}")

    return ret


def dense_knn_matrix(training, x, k=16, relative_pos=None, args=None):
    ...
    with torch.no_grad():
        ...
        if n_points > n_part:
            ...
            for i in range(groups):
                ...
                # _, nn_idx_part = torch.topk(-dist, k=k)
                nn_idx_part = include_reverse(
                    training, dist, k, args.include_reverse)
            ...
            nn_idx = include_reverse(training, dist, k, args.include_reverse)
            logger.debug(
...
class DenseDilatedKnnGraph(nn.Module):
    ...
    def __init__(self, args, k=9, dilation=1, stochastic=False, epsilon=0.0):
    ...
    def forward(self, x, y=None, relative_pos=None):
        if y is not None:
            ...
            x = F.normalize(x, p=2.0, dim=1)
            ####
            edge_index = dense_knn_matrix(self.training,
                                          x, self.k * self.dilation, relative_pos, self.args)
            logger.debug(
