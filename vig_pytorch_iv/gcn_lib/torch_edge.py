...
def include_reverse(dist, k, is_reverse):
    if not is_reverse:
...
def dense_knn_matrix(x, k=16, relative_pos=None, is_reverse: bool = False):
    ...
    with torch.no_grad():
        ...
        if n_points > n_part:
            ...
            for i in range(groups):
                ...
                # _, nn_idx_part = torch.topk(-dist, k=k)
                nn_idx_part = include_reverse(dist, k, is_reverse)
            ...
            nn_idx = include_reverse(dist, k, is_reverse)
            logger.debug(
...
class DenseDilatedKnnGraph(nn.Module):
    ...
    def forward(self, x, y=None, relative_pos=None):
        ...
        ####
        iv_edge_index = dense_knn_matrix(
            x, self.k * self.dilation, relative_pos, False)
        v_edge_index = dense_knn_matrix(
            x, self.k * self.dilation, relative_pos, True)
        logger.debug(
            f"In DenseDilatedKnnGraph, edge_index.shape when y is None: {iv_edge_index.shape}")

        iv_out = self._dilated(iv_edge_index)
        v_out = self._dilated(v_edge_index)
        logger.debug(
            f"In DenseDilatedKnnGraph, out.shape after _dilated: {iv_out.shape}")
