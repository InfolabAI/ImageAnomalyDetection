...
class DeepGCN(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        logger.debug(f"stem:", x.shape)
        B, C, H, W = x.shape

        for i in range(self.n_blocks):
            x = self.backbone[i](x)
            logger.debug(f"block {i}:", x.shape)

        x = F.adaptive_avg_pool2d(x, 1)
        logger.debug(f"adaptive_avg_pool2d:", x.shape)
...
def vig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            ...
            # activation layer {relu, prelu, leakyrelu, gelu, hswish}
            self.act = 'gelu'
            # batch or instance normalization {batch, instance}
            self.norm = 'batch'
    ...
    logger.debug(
        f"pretrained_gnn is applied")
    model.load_state_dict(torch.load(
        '/home/robert.lim/main/other_methods/my_GNN_SimpleNet/vig_pytorch_pretrained/weights/vig_ti_74.5.pth'))
    return model
...
def vig_s_224_gelu(pretrained=False, **kwargs):
    ...
    logger.debug(
        f"pretrained_gnn is applied")
    model.load_state_dict(torch.load(
        '/home/robert.lim/main/other_methods/my_GNN_SimpleNet/vig_pytorch_pretrained/weights/vig_s_80.6.pth'))
    return model
...
def vig_b_224_gelu(pretrained=False, **kwargs):
    ...
    logger.debug(
        f"pretrained_gnn is applied")
    model.load_state_dict(torch.load(
        '/home/robert.lim/main/other_methods/my_GNN_SimpleNet/vig_pytorch_pretrained/weights/vig_b_82.6.pth'))
    return model


def vig_224_gelu(type):
    if type == 'ti':
        ...
        raise ValueError(f"Unknown size: {type}")
